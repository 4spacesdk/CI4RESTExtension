...
<?php if($endpoint->method == 'get') { ?>
    ...
    public find(next?: (value: <?=$endpoint->responseSchema ?? 'any'?>[]) => void) {
        return super.executeFind(next);
    }
<?php } else if($endpoint->method == 'delete') { ?>

    public delete(next?: (value: <?=$endpoint->responseSchema ?? 'any'?>) => void) {
        return super.executeDelete(next);
    }
<?php } else { ?>

    public save(data: <?=$endpoint->requestEntity ?? 'any'?>, next?: (value: <?=$endpoint->responseSchema ?? 'any'?>) => void) {
